...
(function($) {
    ...
    tinymce.PluginManager.add('tma_annotate', function(editor, url) {
        var TMA = {
            'errors': {
                'missing_fields': 'Select the color and the annotation text',
                'missing_annotation': 'Please select some text for creating an annotation',
                'missing_selected': 'Please select the annotation you want to delete',
            },
            'tooltips': {
                'annotation_settings': 'Annotation settings',
                'annotation_create': 'Create annotation',
                'annotation_delete': 'Delete annotation',
                'annotation_hide': 'Hide annotations'
            },
            'settings': {
                'setting_annotation': 'Annotation',
                'setting_background': 'Background color'
            }
        };
                    ...
                    editor.windowManager.open({
                        ...
                        onsubmit: function(e) {
                            if (e.data.annotation) {
                                ...
                                if (editor.getParam("tma_annotate_user_callback")) {
                                    var data = editor.execCallback('tma_annotate_user_callback');
                                    if (data) {
                                        editor.selection.setContent('<span class="annotation" data-author="' + data.ID + '" data-name="' + data.display_name + '" data-annotation="' + dataAnnotation.replace(/"/g, '&quot;') + '" style="background-color:' + e.data.annotationbg + '">' + selectedText + '</span>');
                                    } else {
        ...
        editor.addMenuItem('tma_annotatehide', {
            title: TMA.tooltips.annotation_hide,
            image: url + '/img/annotation-hide.png',
            cmd: 'tma_cmd_hide',
            onPostRender: tma_toggleHide,
            text: TMA.tooltips.annotation_hide
        });
    });
})(jQuery);